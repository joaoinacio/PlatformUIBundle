...
YUI.add('ez-navigationhubview-tests', function (Y) {
    ...
    routeMatchTest = new Y.Test.Case({
        ...
        setUp: function () {
            ...
            this.Item = Y.Base.create('itemTest', Y.eZ.NavigationItemView, [], {
                match: function (route) {
                    that.matchCalls[this.get('identifier')] = route;
        ...
        tearDown: function () {
            ...
            delete this.item3;
            delete this.Item;
            delete this.matchCalls;
        },

        "Should call match on all navigation items": function () {
            var matchedRoute = {name: "externalRoute"};
            this.view.set('matchedRoute', matchedRoute);

            Assert.areEqual(
                3, Y.Object.keys(this.matchCalls).length,
                "match() should have been called 3 times"
            );
            Assert.areSame(
                this.matchCalls.item1, matchedRoute,
                "match() should have been called with the matched route"
            );
            Assert.areSame(
                this.matchCalls.item2, matchedRoute,
                "match() should have been called with the matched route"
            );
            Assert.areSame(
                this.matchCalls.item3, matchedRoute,
                "match() should have been called with the matched route"
            );
        },
